...
spec = do
  describe "getStatement" $ do
      it "solves java correctly" $ do
          ...
          Java.JavaParser.parse sourceCode3 `shouldBe` (ParserResult 1 12 3 1)
          sourceCode4 <- (readFile (currDir ++ "/test/Java/Switch2.java"))  
          Java.JavaParser.parse sourceCode4 `shouldBe` (ParserResult 1 12 3 1)
          sourceCode5 <- (readFile (currDir ++ "/test/Java/IfElse.java"))
          Java.JavaParser.parse sourceCode5 `shouldBe` (ParserResult 1 6 2 1)